...
void plugin_set_window_size(int *w,int *h,const char * plugin_uri) {
    (*w) = 515; //set initial width of main window
    (*h) = 325; //set initial height of main window
}
...
void plugin_create_controller_widgets(X11_UI *ui, const char * plugin_uri) {
    ...
    ui->widget[0] = add_lv2_knob (ui->widget[0], ui->win, 3, "DELAYMILLISECONDS", ui, 60,  43, 185, 198);
    set_adjustment(ui->widget[0]->adj, 20.0, 20.0, 1.0, 40.0, 1.0, CL_CONTINUOS);

    ui->widget[1] = add_lv2_knob (ui->widget[1], ui->win, 4, "IMAGEVOLUME", ui, 245,  213, 139, 80);
    set_adjustment(ui->widget[1]->adj, 0.0, 0.0, -96.0, 0.0, 0.1, CL_CONTINUOS);

    ui->widget[2] = add_lv2_knob (ui->widget[2], ui->win, 5, "LOWPASSFILTERFREQ", ui, 367,  126, 119, 80);
    set_adjustment(ui->widget[2]->adj, 8000.0, 8000.0, 20.0, 20000.0, 100.0, CL_CONTINUOS);

    ui->widget[3] = add_lv2_hslider (ui->widget[3], ui->win, 6, "IMAGEPAN", ui, 346,  71, 120, 30);
    set_adjustment(ui->widget[3]->adj, 20.0, 20.0, -90.0, 90.0, 1.0, CL_CONTINUOS);

    ui->widget[4] = add_lv2_hslider (ui->widget[4], ui->win, 7, "ORIGINALPAN", ui, 303,  20, 185, 30);
